...
func (self *Table) Render(buf *Buffer) error {
	...
	for i, h := range self.Header {
		...
		if width > (self.Width-self.CellXPos[i])+1 {
			continue
		}
	...
	// prints each row
	for rowNum := self.TopRow; rowNum < self.TopRow+self.Height-1 && rowNum < len(self.Rows); rowNum++ {
		...
		if self.Cursor {
			if (self.SelectedItem == "" && rowNum == self.SelectedRow) || (self.SelectedItem != "" && self.SelectedItem == row[self.UniqueCol]) {
				...
				for _, width := range self.ColWidths {
					...
					buf.SetString(1, y, strings.Repeat(" ", self.Width), fg, self.Bg)
				}
...
func (self *Table) calcPos() {
	...
	if self.SelectedRow > self.TopRow+(self.Height-2) {
		self.TopRow = self.SelectedRow - (self.Height - 2)
	}
}
...
func (self *Table) HalfPageUp() {
	self.SelectedRow = self.SelectedRow - (self.Height-2)/2
	self.calcPos()
}
...
func (self *Table) PageUp() {
	self.SelectedRow -= (self.Height - 2)
	self.calcPos()
}
